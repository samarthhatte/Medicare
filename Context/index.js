...
export const StateContextProvider = ({ children }) => {
  ...
  const ADD_MEDICINE = async (medicine) => {
    try {
      ...
      if (!verifyingDoctor) return notifyError("Verifying Doctor is missing");
if (!name) return notifyError("Name is missing");
if (!brand) return notifyError("Brand is missing");
if (!manufacturer) return notifyError("Manufacturer is missing");
if (!manufacturDate) return notifyError("Manufacture Date is missing");
if (!expiryDate) return notifyError("Expiry Date is missing");
if (!code) return notifyError("Code is missing");
if (!companyEmail) return notifyError("Company Email is missing");
if (!discount) return notifyError("Discount is missing");
if (!manufactureAddress) return notifyError("Manufacture Address is missing");
if (!price) return notifyError("Price is missing");
if (!quentity) return notifyError("Quantity is missing");
if (!currentLocation) return notifyError("Current Location is missing");
if (!mobile) return notifyError("Mobile number is missing");
if (!email) return notifyError("Email is missing");
if (!image) return notifyError("Image is missing");
if (!description) return notifyError("Description is missing");
      ...
      if (address) {
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Medicine Added Succesfully");
  ...
  const APPROVE_DOCTOR_STATUS = async (_doctorId) => {
    try {
      ...
      setLoader(true);
      notifySuccess("Registration Approval is processing... ");
      ...
      if (address) {
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Registration Approved Successfully");
          setReCall(reCall + 1);
        }
      }
    } catch (error) {
  ...
  const ADD_PATIENTS = async (patient, doctor) => {
    try {
      ...
      if (!title) return notifyError("Title is missing");
      if (!firstName) return notifyError("First Name is missing");
      if (!lastName) return notifyError("Last Name is missing");
      if (!gender) return notifyError("Gender is missing");
      if (!medicalHistory) return notifyError("Medical History is missing");
      if (!yourAddress) return notifyError("Address is missing");
      if (!mobile) return notifyError("Mobile number is missing");
      if (!phoneRegex.test(mobile))
        return notifyError("Invalid mobile number. Please enter a 10-digit number starting with 6-9.");
      if (!emailID) return notifyError("Email is missing");
      if (!emailRegex.test(emailID))
        return notifyError("Invalid email address. Please enter a valid email.");
      if (!birth) return notifyError("Date of Birth is missing");
      if (!walletAddress) return notifyError("Wallet Address is missing");
      if (!image) return notifyError("Profile image is missing");
      if (!message) return notifyError("Message is missing");
      if (!city) return notifyError("City is missing");
      if (!doctorName) return notifyError("Doctor's Name is missing");
      if (!doctorAddress) return notifyError("Doctor's Address is missing");
  ...
  const BOOK_APPOINTMENT = async (booking, bookingDoctor) => {
    try {
      ...
      const { accountAddress, title, firstName, lastName, doctorID } =
        ...
        if (!from) return notifyError("Sender address (From) is missing");
        if (!to) return notifyError("Recipient address (To) is missing");
        if (!appointmentDate) return notifyError("Appointment date is missing");
        if (!condition) return notifyError("Condition is missing");
        if (!message) return notifyError("Message is missing");
        if (!accountAddress) return notifyError("Your account address is missing");
