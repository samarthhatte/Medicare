...
const networks = {
  holesky: {
    ...
    rpcUrls: ["https://holesky.gateway.tenderly.co"],
...
export const GET_PATIENT_MEDICAL_HISTORY = async (_patientID) => {
  const contract = await HEALTH_CARE_CONTARCT();

  const history = await contract.GET_PATIENT_MEDICAL_HISTORY(_patientID);
...
export const ASK_AI_CHAT = async (prompt) => {
  ...
  try {
    const res = await fetch("https://scaling-trust-ai.onrender.com/api/ask", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    return "Something went wrong calling the backend.";
  }
};


//Clear History Of Ai Chat
export const CLEAR_AI_CHAT_HISTORY = () => {
