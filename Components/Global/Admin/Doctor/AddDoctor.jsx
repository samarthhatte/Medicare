...
const AddDoctor = () => {
  const { ADD_DOCTOR, setLoader, notifySuccess, notifyError } =
    ...
    const handleAddDoctor = () => {
      ...
      if (!phoneRegex.test(doctor.mobile)) {
        notifyError("Invalid mobile number. Please enter a 10-digit number starting with 6-9.");
        return;
      }
    
      if (!emailRegex.test(doctor.emailID)) {
        notifyError("Invalid email address. Please enter a valid email.");
        return;
      }
  ...
  {/* 👇 Updated this to use the validated submit function */}
  <button
